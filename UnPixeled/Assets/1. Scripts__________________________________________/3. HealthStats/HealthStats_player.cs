...
public class HealthStats_player : MonoBehaviour
{
    ...
    void Awake()
    {
        EventGame.playerStatAction.AddListener(HealthStatsAction);
    }
    ...
    void Update()
    {
        RegenerateHealth();
        RegenerateEnergy();
        EventGame.updatePlayerStats.Invoke(health, energy, expirience);
    }
        ...
        if (!disableDeaht)
            switch (type)
            {
                ...
                case "energy":
                    ...
                    {
                        EventGame.disableDash.Invoke(true);
                        EventGame.disableDefence.Invoke(true);
                    }
                    break;
            }
    }
    ...
    void RegenerateEnergy()
    {
        ...
        if (energy > minimumEnergyForDash)
            EventGame.disableDash.Invoke(false);
        if (energy > minimumEnergyForDefence)
            EventGame.disableDefence.Invoke(false);
    }
