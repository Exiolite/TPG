...
public class PlayerManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        EventGame.useHealItem.AddListener(UseHealItem);
    }
    ...
    //InventoryActions
    private void PlayerInventoryAction(Inventory.InventoryActions _action, ItemData _item, int _count)
    {
        playerInventory.InventoryAction(_action, _item, _count);
        EventGame.inventoryUpdate.Invoke();
    }

    private void PlayerEqupmentAction(ItemData _item)
    {
    ...
    private void UseHealItem()
    {
        ...
        {
            if (playerInventory.container[i].item.restoreHealth > 0)
            {
                PlayerEqupmentAction(playerInventory.container[i].item);
                return;
            }
        }
    }
